...
describe('Wss Connection Suite', () => {
  ...
  for (let spec of [
    {
      bind: 'wss://0.0.0.0:0/hello-test',
      travis: true
    },
    {
      bind: 'wss://[::]:0/hello-test',
      travis: false
    }
  ]) {
    describe(spec.bind, () => {
      if (process.env.TRAVIS && !spec.travis) {
        return
      }
      ...
      beforeEach(async function () {
        ...
        f = await new ConnectionBackendWss({
          passphrase: 'hello',
          rsaBits: 1024
        })
        c = f.connectionInterface

        c.on('event', e => b.push(e))

        console.log('binding, may generate cert')
        // await c.bind(spec.bind)
