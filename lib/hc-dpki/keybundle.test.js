...
describe('KeyBundle Suite', () => {
  ...
  it('should encrypt & decrypt one', async () => {
    const blob = await bundle0.encrypt([bundle1.getEncId()], Buffer.from('hello'))
    const res1 = await bundle1.decrypt(bundle0.getEncId(), blob)
    expect(res1.toString()).equals('hello')
  })

  it('should encrypt & decrypt many', async () => {
    const blob = await bundle0.encrypt([
      bundle1.getEncId(),
      bundle2.getEncId()
    ], Buffer.from('hello'))
    ...
    const res2 = await bundle2.decrypt(bundle0.getEncId(), blob)
    expect(res2.toString()).equals('hello')
  })

  it('others should not be able to decrypt', async () => {
    console.log('bundle0', bundle0._encPubId)

    const cipher = await bundle0.encrypt([
      bundle1.getEncId()
    ], Buffer.from('hello'))

    expect(cipher.byteLength).equals(360)

    try {
      await bundle2.decrypt(bundle0.getEncId(), cipher)
    } catch (e) {
      return
    }
    throw new Error('expected exception, got success')
