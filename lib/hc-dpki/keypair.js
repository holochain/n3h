...
class KeyBundle extends AsyncClass {
  ...
  static async fromBlob (blob, passphrase) {
    ...
    console.log(blob)
    let blob0 = Buffer.from(blob[0])
    console.log(blob0)
    return new KeyBundle({
      signPubId: util.encodeSign(blob0),
      encPubId: util.encodeEnc(Buffer.from(blob[1])),
  ...
  async init (opt) {
    ...
    // Check pub key encoding is valid
    const signPub = util.decodeSign(opt.signPubId)
    if (opt.encPub) {
      const encPub = util.decodeEnc(opt.encPubId)
    }
    // Done
  ...
  encrypt (recipientEncIds, data) {
    ...
    for (let recEncId of recipientEncIds) {
      const recEncPub = Buffer.from(util.decodeEnc(recEncId), 'base64')

      console.log(recEncId, '\n', recEncPub)
      console.log(this)
      const thisEncPubId = Buffer.from(this._encPubId, 'base64')
      console.log(thisEncPubId.toString('base64'))
      // XXX lru cache these so we don't have to re-gen every time?
      const { tx } = mosodium.kx.serverSession(
        thisEncPubId, this._encPriv, recEncPub)
  ...
  decrypt (sourceEncId, cipher) {
    ...
    const srcEncPub = Buffer.from(util.decodeEnc(sourceEncId), 'base64')

    // we will call the encryptor the "server"
    // and the recipient (us) the "client"
    // XXX cache?
    const { rx } = mosodium.kx.clientSession(
      Buffer.from(this._encPubId, 'base64'), this._encPriv, srcEncPub)
