...
/**
 * This is Proof-of-Concept code. Not to be used directly.
 * Once we settle on the specific protocol, this should be integrated
 * with the Connection interface, so it can wrap another Connection
 * instance providing layered encryption.
 */
class TransitEncryption extends AsyncClass {
  /**
   * Create a transient kx keypair and our remote map for tracking sessions
   */
  async init () {
    await super.init()
  ...
  /**
   * request a session with a remote (create our transmit session secret)
   */
  ...
  /**
   * accept a remote session request (create OUR transmit session secret)
   */
  async handshakeStep2 (remoteId, handshakeMsg) {
    const othPub = Buffer.from(hck0.decode(remoteId))

    let offset = 0
    const nonce = handshakeMsg.slice(offset, offset + mosodium.aead.NONCE_BYTES)
    offset += mosodium.aead.NONCE_BYTES
    const cipher = handshakeMsg.slice(offset)
  ...
  /**
   * finalize a session we initiated with a remote
   */
  async handshakeStep3 (remoteId, handshakeMsg) {
  ...
  /**
   * send a reliable message to a list of remotes
   */
  async sendReliable (remoteIdList, data) {
    ...
    try {
      ...
      for (let remoteId of remoteIdList) {
        ...
        await ref.reliableNonceTx.increment()
   ...
   */
  async receiveReliable (cipherMessage) {
    const msgKey = await mosodium.SecBuf.secure(mosodium.aead.KEY_BYTES)

    try {
      const recipList = msgpack.decode(cipherMessage)
      ...
      top: for (let recip of recipList) { // eslint-disable-line no-labels
        for (let ref of this._remote.values()) {
          try {
            ...
            await ref.reliableNonceRx.increment()
            success = true
  ...
  /**
   * send an unreliable message to a list of remotes
   */
  async sendUnreliable (remoteIdList, data) {
    ...
    try {
      ...
      for (let remoteId of remoteIdList) {
        ...
        await ref.unreliableNonceTx.increment()
  ...
  /**
   * receive an unreliable message from a remote
   */
  async receiveUnreliable (cipherMessage) {
    const msgKey = await mosodium.SecBuf.secure(mosodium.aead.KEY_BYTES)
      ...
      top: for (let i = 0; i < recipList.length; i += 2) { // eslint-disable-line no-labels
        ...
        for (let ref of this._remote.values()) {
          try {
            ...
            const ndif = await ref.unreliableNonceRx.compare(nonce)
            if (ndif > 0) {
              nonceIgnore = true
              continue
            }
            await ref.unreliableNonceRx.write(0, nonce)
            await ref.unreliableNonceRx.increment()
            success = true
            break top // eslint-disable-line no-labels
          } catch (e) { /* pass */ }
        }
  ...
  /**
   * establish some session data, namely our nonce counters for send/receive
   */
    ...
    if (!this._remote.has(remoteId)) {
      this._remote.set(remoteId, {
        ...
        unreliableNonceTx: await mosodium.SecBuf.ref(Buffer.from(
          '80'.padStart(mosodium.aead.NONCE_BYTES * 2, '0'), 'hex')),
