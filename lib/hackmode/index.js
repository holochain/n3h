...
class N3hHackMode extends N3hMode {
  ...
  async _handleDhtEvent (e) {
    ...
    switch (e.type) {
      ...
      case 'gossipTo':
        ...
        // // log.t('  bundleData = ', parsed)
        // for (const agentId of e.peerList) {
        //   const peerId = this._dht.getPeerLocal(agentId).peerData
        //   log.t('  ids = ', agentId, peerId)
  ...
  _handleP2pEvent (evt) {
    ...
    switch (evt.type) {
      case 'peerConnect':
        ...
        // this.sleep(3000)
        // const peer = this._p2p.getPeer(peerAddress)
        // if (!peer) {
        //   log.w(me + ' unknown peer', this.nick(peerAddress))
        //   break
        // }

        // this._p2p.fetchPeer(peerAddress).then((peer) => {
       // log.w(me + ' peer known', this.nick(peer.peerAddress), peer.peerTransport)


        // log.t(this.me() + '_agent2peer', this._agent2peer)
        // let agentId = ''
        // for (const peerId of this._agent2peer.values()) {
        //   log.t(this.me() + '_agent2peer', peerId)
        //   if (peerId === evt.peerAddress) {
        //     agentId = this._agent2peer.get(peerId)
        //     break
        //   }
        // }
        ...
        this._p2pSend(evt.peerAddress, {
          type: 'gossipAllTracks', // advertise: this._p2p.getAdvertise(),
          dnaByAgent: Array.from(this._ipcDnaByAgent.keys())
        })
        // }, (err) => {
        //   log.e('fetchPeer (' + this.nick(peerAddress) + ') failed', err.toString())
        // })

        // Hold peer in Data DHT
  ...
  _handleP2pPublish (opt) {
    // log.t('@@@@', opt)
    let peerTransport
    log.t('@@' + this.me() + '@@', opt.from.substring(2, 6), opt.data.type)
