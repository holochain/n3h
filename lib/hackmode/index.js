    ...
    switch (data.method) {
      ...
      case 'handleFetchEntryResult':
        ...
        if (bucketId !== '') {
          ...
          log.t(this.me() + ' handleFetchEntryResult insert:', data.dnaAddress, data.address, isPublish)
          this._getMemRef(data.dnaAddress).insert({
            type: 'dhtEntry',
            providerAgentId: data.providerAgentId,
            address: data.address,
  ...
  async _handleDhtEvent (e) {
    ...
    switch (e.type) {
      ...
      case 'dataHoldRequest':
        ...
        if (!isEntryEmpty) {
          this._getMemRef(data.dnaAddress).insert({
            type: 'dhtEntry', providerAgentId: data.providerAgentId, address: data.entryAddress, content: data.entry
          })
        }
