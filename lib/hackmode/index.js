...
class N3hHackMode extends N3hMode {
   ...
   */
  _handleP2pEvent (evt) {
    log.t('(' + this._p2p.getId().substring(2, 6) + ')._handleP2pEvent()', evt.type)
    switch (evt.type) {
      case 'peerConnect':
    ...
    switch (e.type) {
      ...
      case 'gossipTo':
        ...
        for (const peerId of e.peerList) {
          // const peerId = this._dht.getPeerLocal(agentId).peerTransport
  ...
  //  * @private
  //  */
  // async _p2pSendWithType (peerAddress, type, obj) {
  //   return this._p2p.publishReliableWithType(
  //     [peerAddress],
  //     type,
  //     msgpack.encode(obj).toString('base64')
  //   )
  ...
  _handleP2pPublish (opt) {
    // log.t('@@@@', opt)
    log.t('@@@@', opt.from.substring(2, 6), opt.data.type)
