...
class N3hHackMode extends N3hMode {
  ...
  async _handleDhtEvent (e) {
    log.t(this.me() + '._handleDhtEvent', e)
    // Handle event by type
    switch (e.type) {
      case 'peerHoldRequest':
        // issued by _dht._handlePeerMap()
        log.t(this.me() + 'PEER INDEXED', e)
      ...
      case 'dataHoldRequest':
        ...
        log.t(this.me() + '  dataHoldRequest: ', data)
      ...
      case 'dataFetch':
        ...
        let content = null
        // let content = Buffer.from(JSON.stringify(null)).toString("base64")
    ...
    if (!(dnaAddress in this._memory)) {
      ...
      mem.registerIndexer((store, data) => {
        if (data && data.type === 'dhtEntry') {
          ...
          log.t('got dhtEntry ; contentHash', contentHash, getHash(data.address))
