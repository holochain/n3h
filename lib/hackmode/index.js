...
class N3hHackMode extends N3hMode {
  ...
  /**
   * _dht is notifying me of events
   * @private
   */
  async _handleDhtEvent (e) {
    ...
    switch (e.type) {
      ...
      case 'dataHoldRequest':
        // received dataHoldRequest from gossip decoded by our _dht
        // Sender: dataFetch
      ...
      case 'dataFetch':
        ...
        for (const dnaAddress of this._memory) {
          const mem = this._getMemRef(dnaAddress)
          if (mem.has(e.dataAddress)) {
            content = mem.get(e.dataAddress)
            content.entryAddress = e.dataAddress
            // FIXME content.providerAgentId =
            // FIXME content.dnaAddress =
