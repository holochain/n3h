...
class N3hHackMode extends N3hMode {
  ...
  _getMemRef (dnaAddress) {
    if (!(dnaAddress in this._memory)) {
      ...
      this._memory[dnaAddress] = {
        mem,
        agentToPeerAddress: mem.registerIndexer((store, data) => {
  ...
  /**
   * We can't remove a mem entry but we can update it, so we update the peerAddress to undefined
