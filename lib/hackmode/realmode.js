...
class N3hRealMode extends N3hMode {
  ...
  async _handleIpcJson (data, ipcSenderId) {
    ...
    switch (data.method) {
      ...
      case 'publishEntry':
        ...
        let reqId = 'publishEntry_' + this.nick(data.entryAddress)
        ...
        for (const entryAspect of data.aspectList) {
          this._bookkeepAspect(this._authoredEntryBook, chainId, data.entryAddress, entryAspect.aspectAddress)
          // publish
      ...
      case 'handleFetchEntryResult':
        ...
        this._dhtEntryRequestPendingList.delete(data.entryAddress)
        let entryData = []
        for (const entryAspect of data.aspectList) {
          log.t(this.me() + ' handleFetchEntryResult insert:', data)
          this._bookkeepAspect(this._authoredEntryBook, chainId, data.entryAddress, entryAspect.address)
          const aspectData = {
            type: 'entryAspect',
            providerAgentId: data.providerAgentId,
            entryAddress: data.entryAddress,
  ...
  async _handleChainDhtEvent (e, chainId) {
    ...
    switch (e.type) {
      ...
      case 'peerHoldRequest':
        ...
        let newSet = peerTracks.has(dnaAddress) ? new Set(peerTracks.get(dnaAddress)) : new Set()
        newSet.add(toHoldAgentId)
        peerTracks.set(dnaAddress, Array.from(newSet))
        ...
        // console.error("@@@@@", JSON.stringify(Object.entries(this._peerTracks).map(([k, v]) => ({[k]: Array.from(v.entries())})), null, 2))
        break
      case 'gossipTo':
  ...
  _handleP2pEvent (evt) {
    ...
    switch (evt.type) {
      case 'peerConnect':
        if (this._p2p.getId() === evt.peerAddress) {
          log.w('why are we getting a peerConnect to ourselves? aborting')
          return
        }
      ...
      case 'handleSendMessage':
        ...
        if (!this._hasTrack(opt.data.toAgentId, opt.data.dnaAddress)) {
          ...
          this._p2pSend(mId, {
            ...
            resultInfo: Buffer.from('Agent ' + opt.data.toAgentId + ' is not tracking DNA: ' + opt.data.dnaAddress).toString('base64')
          })
          return
        }
        // forward to Core
    ...
    if (!(chainId in this._memory)) {
      ...
      mem.registerIndexer((store, data) => {
        if (data && data.type === 'entryAspect') {
          ...
          getDebugSnapshot().insert({
            [data.aspectAddress]: {
              aspectAddress: data.aspect.aspectAddress,
              typeHint: data.aspect.typeHint,
              publishTs: data.aspect.publishTs,
              aspect: Buffer.from(data.aspect.aspect).toString()
            }
          }, 'storeAspect', {
            'entryAddress': data.entryAddress,
            'aspectAddress': data.aspectAddress,
            '$cas$aspect': data.aspectAddress
          })
  ...
  _getMachineIdOrFail (chainId, receiverAgentId, requestId) {
    ...
    if (chainDht === undefined || !chainDht.getPeerLocal(receiverAgentId)) {
      ...
      this._ipcSend({
        ...
        resultInfo: Buffer.from('No routing found for agent id "' + receiverAgentId + '"').toString('base64')
      })
      return null
    }
  ...
  async _untrack (dnaAddress, agentId) {
    ...
    getDebugSnapshot().insert({}, 'untrack', {
      dnaAddress,
      agentId
    })
  }
