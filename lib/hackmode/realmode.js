...
class N3hRealMode extends N3hMode {
  ...
  async _sendToAgent (msg, dnaAddress, agentId, toAgentId, fromAgentId, reqId) {
    const mId = this._getPeerAddressOrFail(dnaAddress, agentId, toAgentId, fromAgentId, reqId)
    if (mId === null) {
      return
    }
  ...
  async _handleIpcJson (data, ipcSenderId) {
    ...
    switch (data.method) {
      case 'failureResult':
        ...
        this._sendToAgent(msg, data.dnaAddress, this._deconstructChainId(chainId)[1], data.toAgentId)
        return
      case 'requestState':
        this._ipcSend('json', {
          method: 'state',
      ...
      case 'sendMessage':
        ...
        this._sendToAgent(msg, data.dnaAddress, data.fromAgentId, data.toAgentId, data.fromAgentId, data._id)
        return
      ...
      case 'handleFetchEntryResult':
        ...
        this._sendToAgent(msg, data.dnaAddress, data.providerAgentId, data.requesterAgentId, data.providerAgentId, data._id)
        return
      case 'handleGetAuthoringEntryListResult':
        ...
        for (const [entryAddress, aspectAddressList] of data.addressMap) {
          for (const aspectAddress of aspectAddressList) {
            // If one aspectAddress is unknown, fetch Entry
            if (!knownAuthoringEntryList.includes(entryAddress) ||
      ...
      case 'handleGetGossipingEntryListResult':
        ...
        for (const [entryAddress, aspectAddressList] of data.addressMap) {
          for (const aspectAddress of aspectAddressList) {
            this._bookkeepAspect(this._storedEntryBook, chainId, entryAddress, aspectAddress)
          }
        }
        return
    }
   ...
   * @private
   */
  async _handleDhtEvent (e, dnaAddress, agentId) {
    // log.t(this.me() + '._handleDhtEvent(' + e.type + ') - ' + dnaAddress + ' + ' + agentId)
  ...
  _handleP2pPublish (opt) {
    ...
    switch (opt.data.type) {
      case 'dnaDhtGossip':
        ...
        for (const [agentId, dnaList] of this._ipcDnaByAgent) {
        ...
        for (const [agentId, dnaList] of this._ipcDnaByAgent) {
          for (const dna of dnaList) {
            if (dna === opt.data.dnaAddress) {
              ...
              this._handleDhtEvent(peerHoldEvent, opt.data.dnaAddress, agentId)
              break
            }
          }
        }
      ...
      case 'gossipAllTracks':
        ...
        for (const [agentId, dnaAddress] of opt.data.dnaByAgent) {
          ...
          for (const [agentId, dnaList] of this._ipcDnaByAgent) {
            for (const dna of dnaList) {
              if (dna === dnaAddress) {
                this._handleDhtEvent(peerHoldEvent, dnaAddress, agentId)
                break
              }
            }
          }
      ...
      case 'handleSendMessage':
        ...
        if (!this._ipcHasTrack(opt.data.toAgentId, opt.data.dnaAddress)) {
          ...
          const mId = this._getPeerAddressOrFail(opt.data.dnaAddress, opt.data.fromAgentId, opt.from, opt.data.fromAgentId, opt.data._id)
          if (mId === null) {
            return
          }
          this._p2pSend(mId, {
  ...
  async _initDht (dnaAddress, agentId) {
    ...
    // Set event handler
    dht.on('event', e => this._handleDhtEvent(e, dnaAddress, agentId).catch(err => {
    ...
    if (!(chainId in this._memory)) {
      ...
      mem.registerIndexer((store, data) => {
        if (data && data.type === 'entryAspect') {
          ...
          if (this._hasEntryAspect(chainId, data.entryAddress, data.address)) {
            log.t(this.me() + ' entryAspect is known:', data.address)
            return
          }
          // log.t(this.me() + ' dhtEntry is unknown', data.address)
          // bookkeep
          // const chainId = this._intoChainId(dnaAddress, data.providerAgentId)
          this._bookkeepAspect(this._storedEntryBook, chainId, data.entryAddress, data.address)
   ...
   *  FIXME
   */
  _getPeerAddressOrFail (dnaAddress, agentId, receiverAgentId, senderAgentId, requestId) {
    const chainId = this._intoChainId(dnaAddress, agentId)
    // const recvPeer = this._getDhtRef(chainId).getPeerLocal(receiverAgentId)
