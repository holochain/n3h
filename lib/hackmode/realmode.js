...
class N3hRealMode extends N3hMode {
  ...
  async _handleIpcJson (data, ipcSenderId) {
    ...
    switch (data.method) {
      case 'failureResult':
        ...
        tId = await this._getTransportIdOrFail(data.dnaAddress, data.toAgentId)
        if (tId === null) {
          return
        }
        this._p2pSend(tId, {
      ...
      case 'sendMessage':
        ...
        tId = await this._getTransportIdOrFail(data.dnaAddress, data.toAgentId, data.fromAgentId, data._id)
        if (tId === null) {
          return
        }
        this._p2pSend(tId, {
      ...
      case 'fetchEntry':
        ...
        dht = await this._getDhtRef(data.dnaAddress)
        result = await dht.fetchDataLocal(data.address)
      ...
      case 'handleFetchEntryResult':
        ...
        tId = await this._getTransportIdOrFail(data.dnaAddress, data.requesterAgentId, data.providerAgentId, data._id)
        if (tId === null) {
          return
        }
        this._p2pSend(tId, {
      ...
      case 'fetchMeta':
        ...
        // TODO: might need to do _dht.post(DhtEvent.fetchData) instead for non-fullsync DHT
        dht = await this._getDhtRef(data.dnaAddress)
        result = await dht.fetchDataLocal(data.entryAddress)
        if (result) {
          result = JSON.parse(Buffer.from(result, 'base64').toString('utf8'))
  ...
  async _handleDhtEvent (e, dnaAddress) {
    ...
    switch (e.type) {
      case 'peerHoldRequest':
        // issued by _dht._handlePeerMap() , i.e. when receiving Peer data from gossip
        ...
        if (!(dnaAddress in this._dht)) {
          log.w('Received peerHoldRequest for untracked DNA', dnaAddress)
          break
        }
        dht = await this._getDhtRef(dnaAddress)
        log.t(this.me() + ' received PEER data', dht)
        dht.post(e)
      ...
      case 'dataFetch':
        ...
        if (!mem) {
          ...
          dht = await this._getDhtRef(dnaAddress)
          dht.post(Dht.DhtEvent.dataFetchResponse(e.msgId, nullB64))
          break
        }
        ...
        dht = await this._getDhtRef(dnaAddress)
        dht.post(Dht.DhtEvent.dataFetchResponse(e.msgId, dhtData))
        break
      default:
  ...
  _handleP2pPublish (opt) {
    ...
    switch (opt.data.type) {
      case 'dhtGossip':
        ...
        this._getDhtRef(opt.data.dnaAddress).then((dht) => {
          dht.post(DhtEvent.remoteGossipBundle(opt.from, opt.data.bundle))
        }, (err) => {
          log.e('dhtGossip Error', err)
        })
        return
      case 'gossipNewTrack':
      ...
      case 'handleSendMessage':
        ...
        if (!this._ipcHasTrack(opt.data.toAgentId, opt.data.dnaAddress)) {
          ...
          this._getTransportIdOrFail(opt.data.dnaAddress, opt.data.fromAgentId).then((tId) => {
            if (tId === null) {
              return
            }
            this._p2pSend(tId, {
            ...
            log.e('handleSendMessage Error', err)
          })
        }
        return
  ...
  async _initDht (dnaAddress, agentId) {
    ...
    wait.push((async () => {
      ...
      log.t(this.me() + '._initDht @Ctor@')

      let dht = await new Dht(DhtBackendFullsync, dhtInitOptions) // .then((dht) => {

      // log.t(this.me() + '._initDht @Ctor@ -', JSON.stringify(dht))
      ...
      // Done
      // log.t(this.me() + '_initDht', dht.getPeerLocal(agentId))
      // log.t(dht)
      // log.t(this.me() + '_initDht', JSON.stringify(dht))

      this._dht[dnaAddress] = dht

      log.t(this.me() + '_initDht @Ctor@ DONE')
    })())
    await Promise.all(wait)
    log.t(this.me() + '_initDht UH-OH')
  }

  /**
   * @private
   */
  async _getDhtRef (dnaAddress, agentId) {
    log.t(this.me() + '.1._getDhtRef', dnaAddress)

    if (!dnaAddress || typeof dnaAddress !== 'string' || !dnaAddress.length) {
      throw new Error('cannot _getDhtRef without dnaAddress string')
    }
    log.t(this.me() + '.2._getDhtRef', dnaAddress, agentId)

    if (!(dnaAddress in this._dht)) {
    // if (!this._dht.has(dnaAddress)) {
      ...
      log.t(this.me() + '._getDhtRef @adding@ -', dnaAddress, agentId)

      // this._dht[dnaAddress] =
      await this._initDht(dnaAddress, agentId)

      log.t(this.me() + '._getDhtRef. set done', dnaAddress)
      log.t(this.me() + '._getDhtRef. set done', this._dht[dnaAddress])
    }

    log.t(this.me() + '._getDhtRef DONE')
    // return this._dht.get(dnaAddress)
    return this._dht[dnaAddress]
  }
    ...
    if (!(dnaAddress in this._memory)) {
      ...
      mem.registerIndexer((store, data) => {
        if (data && data.type === 'dhtEntry') {
          ...
          this._getDhtRef(dnaAddress).then((dht) => {
            dht.post(DhtEvent.dataHoldRequest(data.address, contentHash))
            ...
            log.e('Insert dhtEntry Error', err)
          })
        }
      })
      ...
      mem.registerIndexer((store, data) => {
        if (data && data.type === 'dhtMeta') {
          ...
          this._getDhtRef(dnaAddress).then((dht) => {
            ...
            log.e('Insert dhtMeta Error', err)
          })
        }
      })
   ...
   *  @private
   */
  async _getTransportIdOrFail (dnaAddress, receiverAgentId, senderAgentId, requestId) {
    const dht = await this._getDhtRef(dnaAddress)
    const peer = dht.getPeerLocal(receiverAgentId)
    if (!peer) {
  ...
  async _track (dnaAddress, agentId) {
    ...
    const dht = await this._getDhtRef(dnaAddress, agentId)
    if (!dht || !(await this._getDhtRef(dnaAddress))) {
      throw new Error('initDht FAILED')
    }
    log.t('' + dht)
    // log.t('' + JSON.stringify(dht))
    // log.t(this.me() + '_track', JSON.stringify(this._dht[dnaAddress]))
    const myself = dht.getPeerLocal(agentId)
    log.t(this.me() + ' REGISTER AGENT', dnaAddress, myself)
