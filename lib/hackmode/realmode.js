...
class N3hRealMode extends N3hMode {
    ...
    switch (e.type) {
      ...
      case 'gossipTo':
        ...
        for (const peerAddress of e.peerList) {
          // if peerAddress is actually an agentId, get agent's real peerAddress
          let mId = peerAddress
          const peer = this._getDhtRef(dnaAddress).getPeerLocal(peerAddress)
          if (peer) {
            mId = peer.peerTransport.substring(5) // remove 'hc://'
            // log.t(this.me() + ' >> ' + peerAddress + this.nick(mId) + ' - dhtGossip')
          }
          // else {
          //   log.t('\'gossipTo\' peerAddress is not an agentId')
          //   log.t(this.me() + ' >> ' + peerAddress + ' - dhtGossip')
          // }
          this._p2pSend(mId, {
  ...
  _handleP2pPublish (opt) {
    ...
    switch (opt.data.type) {
      ...
      case 'gossipNewTrack':
        ...
        // Add peer to dht if we also track the same DNA (this will Bookkeep agentId -> peerAddress)
      ...
      case 'gossipAllTracks':
        ...
        for (const [agentId, dnaAddress] of opt.data.dnaByAgent) {
          ...
          // Bookkeep agentId -> peerAddress
          // Add peer to dht (this Bookkeep agentId -> peerAddress)
  ...
  _getPeerAddressOrFail (dnaAddress, receiverAgentId, senderAgentId, requestId) {
    ...
    const recvPeerAddress = recvPeer.peerTransport.substring(5) // remove 'hc://'
