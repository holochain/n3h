...
class N3hRealMode extends N3hMode {
  async init (workDir, rawConfigData, terminate) {
    ...
    //
    this._dhtEntryRequestPending = new Set()
    //
    ...
    this.$pushDestructor(async () => {
      ...
      this._dhtEntryRequestPending.clear()
      this._dhtEntryRequestPending = null
   ...
   * @param chainId - Sender space and agentId
   * @param toAgentId - Receiver agentId
   * @param reqId - Optional requestId it is responding to.
   * @private
   */
  async _sendToAgent (chainId, toAgentId, msg, reqId) {
    const mId = this._getMachineIdOrFail(chainId, toAgentId, reqId)
    if (mId === null) {
      log.w('No known machineId for agent ' + toAgentId + ' in space ' + chainId)
      return
    }
  ...
  async _handleIpcJson (data, ipcSenderId) {
    ...
    switch (data.method) {
      ...
      case 'handleFetchEntryResult':
        ...
        this._dhtEntryRequestPending.delete(data.entry.entryAddress)
        let entryData = []
        for (const entryAspect of data.entry.aspectList) {
          log.t(this.me() + ' handleFetchEntryResult insert:', data)
        ...
        // // Forward result to requesting agent
        ...
        // }
        // this._sendToAgent(chainId, data.requesterAgentId, msg, data._id)
        return
      case 'handleGetAuthoringEntryListResult':
        ...
        log.d('addressMap', data.addressMap)
        ...
        for (const entryAddress in data.addressMap) {
          ...
          log.d('addressMap.entryAddress', entryAddress)
      ...
      case 'handleGetGossipingEntryListResult':
        ...
        chainId = this._checkRequest(data._id)
        if (chainId === '') {
          log.w('Received handleGetGossipingEntryListResult from unknown request')
          return
        }
        // Update my book-keeping on what this agent has.
        for (const entryAddress in data.addressMap) {
  ...
  async _handleChainDhtEvent (e, chainId) {
    ...
    switch (e.type) {
      ...
      case 'dataHoldRequest':
        for (let item of e.dataList) {
          ...
          if (entryAspect.type !== 'entryAspect') {
            log.e('dataHoldRequest of unknown type', entryAspect.type)
          } else {
            this._getMemRef(chainId).insert({
              type: 'entryAspect',
      ...
      case 'dataFetch':
        ...
        if (this._dhtEntryRequestPending.has(entryAddress)) {
          this._getDhtRef(chainId).post(Dht.DhtEvent.dataFetchResponse(e.msgId, []))
          break
        }
        // Ask core
        log.t('_dhtRequestBook: ', e.msgId, chainId)
        this._dhtRequestBook.set(e.msgId, chainId)
        this._dhtEntryRequestPending.add(entryAddress)
        let fetchEntry = {
          method: 'handleFetchEntry',
