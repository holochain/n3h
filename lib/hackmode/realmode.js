...
class N3hRealMode extends N3hMode {
  ...
  async _sendToAgent (msg, dnaAddress, toAgentId, fromAgentId, reqId) {
    const mId = this._getPeerAddressOrFail(dnaAddress, toAgentId, fromAgentId, reqId)
    if (mId === null) {
      return
    }
  ...
  async _handleIpcJson (data, ipcSenderId) {
    ...
    switch (data.method) {
      case 'failureResult':
        ...
        this._sendToAgent(msg, data.dnaAddress, data.toAgentId)
        return
      case 'requestState':
        this._ipcSend('json', {
          method: 'state',
      ...
      case 'sendMessage':
        ...
        this._sendToAgent(msg, data.dnaAddress, data.toAgentId, data.fromAgentId, data._id)
        return
      ...
      case 'publishEntry':
        ...
        for (const entryAspect of data.entry.aspectList) {
          this._bookkeepAspect(this._publishedEntryBook, data.dnaAddress, data.entry.entryAddress, entryAspect.aspectAddress)
          // publish
          ...
          this._getMemRef(data.dnaAddress).insert({
            ...
            address: entryAspect.aspectAddress,
            aspect: {
              address: entryAspect.aspectAddress,
      ...
      case 'handleQueryEntryResult':
        ...
        // #fullsync hack: send response back to self (ipc)
      ...
      case 'handleFetchEntryResult':
        ...
        chainId = this._checkRequest(data._id)
        if (chainId !== '') {
          const isPublish = data._id === '__publish' // FIXME: extract 'isPublish' from requestId cipher
          const book = isPublish ? this._publishedEntryBook : this._storedEntryBook
          for (const entryAspect of data.entry.aspectList) {
            log.t(this.me() + ' handleFetchEntryResult insert:', data, isPublish)
            this._bookkeepAspect(book, data.dnaAddress, data.entry.entryAddress, entryAspect.aspectAddress)
            this._getMemRef(data.dnaAddress).insert({
              type: 'entryAspect',
              providerAgentId: data.providerAgentId,
              entryAddress: data.entry.entryAddress,
        ...
        // #fullsync hack: send result back to self (ipc) --- FIXME ??
        msg = {
        ...
        this._sendToAgent(msg, data.dnaAddress, data.requesterAgentId, data.providerAgentId, data._id)
        return
        // case 'fetchMeta':
        ...
        //       resultInfo: 'Entry not found during fetchMeta for:' + data.entryAddress
        //     }
        //   }
        ...
        //         continue
        //       }
        //       this._bookkeepAspect(isPublish ? this._publishedMetaBook : this._storedMetaBook, data.dnaAddress, metaId)
      ...
      case 'handleGetAuthoringEntryListResult':
        ...
        if (chainId in this._publishedEntryBook) {
          knownPublishingList = this._publishedEntryBook[chainId]
        }
      ...
      case 'handleGetGossipingEntryListResult':
        ...
        if (chainId in this._storedEntryBook) {
          knownHoldingList = this._storedEntryBook[chainId]
        }
        // FIXME
    ...
    switch (e.type) {
      ...
      case 'dataHoldRequest':
        for (let item of e.dataList) {
          ...
          log.t(this.me() + 'dataHoldRequest - ' + JSON.stringify(parsed))
          if (parsed.type !== 'entryAspect') {
            log.e('dataHoldRequest of unknown type', parsed.type)
          } else {
      ...
      case 'handleSendMessage':
        ...
        if (!this._ipcHasTrack(opt.data.toAgentId, opt.data.dnaAddress)) {
          ...
          this._p2pSend(mId, {
            ...
            resultInfo: 'Agent "' + opt.data.toAgentId + '" is not tracking DNA' + opt.data.dnaAddress
          })
          return
        }
        // forward to Core
  ...
  _getMemRef (dnaAddress) {
    if (!(dnaAddress in this._memory)) {
      ...
      // send IPC handleStoreEntry on inserting an entryAspect
      mem.registerIndexer((store, data) => {
        if (data && data.type === 'entryAspect') {
          log.t(this.me() + ' got entryAspect', data)

          const encodedAspect = Buffer.from(data.aspect).toString('base64')
          this._getDhtRef(dnaAddress).post(DhtEvent.dataHoldRequest(data.entryAddress, [encodedAspect]))

          // Store in Core if it isn't already holding it
          if (this._hasEntry(dnaAddress, data.address)) {
            log.t(this.me() + ' entryAspect is known:', data.address)
            return
          }
          // log.t(this.me() + ' dhtEntry is unknown', data.address)
          // bookkeep
          this._bookkeepAspect(this._storedEntryBook, dnaAddress, data.address)
  ...
  _getPeerAddressOrFail (dnaAddress, receiverAgentId, senderAgentId, requestId) {
    ...
    if (!recvPeer) {
      ...
      this._ipcSend('json', {
        ...
        resultInfo: 'No routing for agent id "' + receiverAgentId + '"'
      })
      return null
    }
  ...
  _createRequest (dnaAddress, agentId) {
    return this._createRequestWithChainId(dnaAddress)
  }
   ...
   * @private
   */
  _bookkeepAspect (book, dnaAddress, entryAddress, aspectAddress) {
    if (!(dnaAddress in book)) {
      book[dnaAddress] = []
    }
    let aspects
    if (!(entryAddress in book[dnaAddress])) {
      aspects = new Set()
    } else {
      aspects = book[dnaAddress].get(entryAddress)
    }
    aspects.push(aspectAddress)
  }
