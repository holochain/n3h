...
describe('hackmode module p2p peer backend Suite', () => {
  ...
  const newBasic = async (workDir) => {
    workDir || (workDir = '')
    const node = (await new P2pBackendHackmodePeer({
      dht: JSON.parse(dht),
  ...
  it('integration', async () => {
    try {
      ...
      const nodeNat = (await new P2pBackendHackmodePeer({
        dht: JSON.parse(dht),
        connection: {
          passphrase: 'hello',
          rsaBits: 1024
  ...
  it('node loadKey', async () => {
    ...
    try {
      ...
      let bundleB = await nodeBase._backend._loadKey(dir)
