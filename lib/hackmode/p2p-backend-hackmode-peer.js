...
function appendStack (prev) {
  prev || (prev = [])
  return prev.concat(['--'].concat(getStack()))
}
...
class P2pBackendHackmodePeer extends AsyncClass {
  ...
  async transportConnect (remoteAdvertise) {
    return this._ensureConnection(remoteAdvertise, getStack())
  }
  ...
  async _publish (msgId, fromPeerAddress, peerAddressList, type, data) {
    ...
    const results = await Promise.all(peerAddressList.map(
      a => this._ensureCIdForPeerAddress(a, stack).catch(e => {
  ...
  /**
   */
  async _getExistingCidForPeerAddress (peerAddress) {
    if (this._conByPeerAddress.has(peerAddress)) {
      return {
    ...
    for (let ref of Array.from(this._con.keys()).map(k => this._con.get(k))) {
      if (ref.peerAddress === peerAddress) {
        this._conByPeerAddress.set(peerAddress, ref.id)
        return {
          cId: ref.id,
          isNew: false
        }
    ...
    if (this._newConTrack.has(peerAddress)) {
      return {
        cId: (await this._newConTrack.get(peerAddress)).cId,
        isNew: false
      }
    }
   ...
   */
  async _ensureConnection (remoteAdvertise, stack) {
    ...
    const shortCircuit = await this._getExistingCidForPeerAddress(
      remotePeerAddress)
    ...
    getDebugSnapshot().insert({}, 'init-connect', {
      peerAddress: remotePeerAddress,
      uri: uri.href,
      peerAddressToCId: Array.from(this._conByPeerAddress.entries()),
      state: Array.from(this._con.keys()).map(k => this._con.get(k)),
      stack: appendStack(stack)
    })
   ...
   * (Called by _publish())
   */
  async _ensureCIdForPeerAddress (peerAddress, stack) {
    ...
    switch (uri.protocol) {
      case 'wss:':
        ...
        return this._ensureConnection(remotePeerAddress, appendStack(stack))
      case 'holorelay:':
        return this._ensureCIdForPeerAddress(uri.hostname, appendStack(stack))
      default:
  ...
  async _removeConnection (connectionId) {
    for (let [peerAddress, cId] of this._conByPeerAddress) {
      if (connectionId === cId) {
        ...
        getDebugSnapshot().insert({}, 'disconnect', {
          cId,
          peerAddress,
          state: Array.from(this._con.keys()).map(k => this._con.get(k))
        })
  ...
  async _handleMessage (cId, data) {
    ...
    switch (type) {
      case '$advertise$':
        ...
        getDebugSnapshot().insert({}, 'connect', {
          cId,
          peerAddress: remotePeerInfo.peerAddress,
          peerAddressToCId: Array.from(this._conByPeerAddress.entries()),
          state: Array.from(this._con.keys()).map(k => this._con.get(k))
        })
