  ...
  async _handleDhtEvent (e) {
    ...
    switch (e.type) {
      ...
      case 'peerHoldRequest':
        ...
        break
      case 'peerTimedOut':
        log.t(this._me() + '._p2p.peerTimedOut:', e)
        // Notify owner of peer disconnection
        await this._spec.$emitEvent(P2pEvent.peerDisconnect(e.peerAddress))
        break
      default:
  ...
  async _handleMessage (cId, data) {
    ...
    switch (type) {
      case '$advertise$':
        ...
        // log.t(this._me() + '._p2p._handleMessage(): $advertise$', this._nick(remotePeerInfo.peerAddress), remotePeerInfo.peerTransport, remotePeerInfo.peerData)
        ...
        await this._spec.$emitEvent(P2pEvent.peerConnect(remotePeerInfo.peerAddress))
        break
      case '$gossip$':
