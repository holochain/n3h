...
class P2pBackendHackmodePeer extends AsyncClass {
  ...
  getPeer (peerId) {
    const res = this._dht.getPeerLocal(peerId)
    log.t(this._me() + '.getPeer()', this._nick(peerId), res)
    return res
  }

  async fetchPeer (peerId) {
    const res = await this._dht.fetchPeer(peerId)
    log.t(this._me() + '.fetchPeer()', this._nick(peerId), res)
    return res
  }
  ...
  //  */
  // async publishReliableWithType (peerAddressList, type, data) {
  //   return this._publish(
  //     null, // no need for a msgId
  //     this._keypair.getId(), // peerAddress is peer's pub key
  //     peerAddressList,
  //     type,
  ...
  async publishReliable (peerAddressList, data) {
    return this._publish(
      ...
      this._keypair.getId(), // peerAddress is peer's pub key
      peerAddressList,
      '$publish$', // publish is of type 'publish'
      Buffer.from(data, 'base64')
    )
  }
  ...
  async _handleDhtEvent (e) {
    // log.t(this._me() + '._p2p._handleDhtEvent()', e)
  ...
  async _handleMessage (cId, data) {
    ...
    // log.t(this._me() + '._p2p._handleMessage(): from & type', this._nick(fromPeerAddress), type)
    log.t(this._me() + '._p2p._handleMessage():', type)
    ...
    switch (type) {
      case '$advertise$':
        ...
        await this._spec.$emitEvent(P2pEvent.peerConnect(rem.peerAddress, rem.peerTransport))
        break
      case '$gossip$':
