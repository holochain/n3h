...
class N3hMode extends AsyncClass {
   ...
   * @private
   */
  _translateClientProtocol (message) {
    switch (message.lib3h_client_protocol) {
      case 'SuccessResult':
        return {
          method: 'successResult',
          _id: message.request_id,
          dnaAddress: message.space_address,
          toAgentId: message.to_agent_id,
          resultInfo: message.result_info
        }
      case 'FailureResult':
        return {
      ...
      case 'HandleSendDirectMassageResult':
        return {
          method: 'handleSendMessageResult',
          dnaAddress: message.space_address,
          _id: message.request_id,
          toAgentId: message.to_agent_id,
          fromAgentId: message.from_agent_id,
      ...
      */
      case 'HandleFetchEntryResult':
        return {
          method: 'handleFetchEntryResult',
          dnaAddress: message.space_address,
          providerAgentId: message.provider_agent_id,
          _id: message.request_id,
          entry: this._translateEntryData(message.entry)
        }
      case 'PublishEntry':
        throw new Error('arg! these are multi')
        return {
          method: 'publishEntry',
          dnaAddress: message.space_address,
          providerAgentId: message.provider_agent_id,
      ...
      default:
        throw new Error('unrecognized lib3h_client_protocol type: ' + message.lib3h_client_protocol)
    }
  }
    ...
    switch (ev.type) {
      ...
      case 'message':
        ...
        try {
          ...
          this._handleIpcJson(this._translateClientProtocol(message), ev.id);
        } catch (e) {
