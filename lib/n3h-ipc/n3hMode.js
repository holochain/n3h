...
class N3hMode extends AsyncClass {
   ...
   */
  _handleIpcEvent (ev) {
    // log.t(this.me() + ' received from Core', ev)
    switch (ev.type) {
      case 'bind':
  ...
  _hasTrackOrFail (agentId, dnaAddress, requestId) {
    ...
    if (this._ipcHasTrack(agentId, dnaAddress)) {
      log.t(this.me() + ' oooo HasTrack() CHECK OK for (agent) "' + agentId + '" -> (DNA) "' + dnaAddress + '"')
      return true
    }
    // Send FailureResult back to IPC
    log.e(this.me() + ' #### HasTrack() CHECK FAILED for (agent) "' + agentId + '" -> (DNA) "' + dnaAddress + '"')
    this._ipcSend('json', {
  ...
  _getTransportIdOrFail (dnaAddress, receiverAgentId, senderAgentId, requestId) {
    ...
    //   log.t('oooo CHECK OK for (agent)"' + receiverAgentId + '" + (DNA) "' + dnaAddress + '" = ' + ref.agentToTransportId[receiverAgentId])
    //   return ref.agentToTransportId[receiverAgentId]
    // }

    const peer = this._dht.getPeerLocal(receiverAgentId)
    if (peer) {
      log.t('oooo CHECK OK for (agent)"' + receiverAgentId + '" + (DNA) "' + dnaAddress + '" = ' + peer.peerTransport.substring(2,6))
      return peer.peerTransport
    }
