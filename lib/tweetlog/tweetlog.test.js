...
describe('tweetlog Suite', () => {
  beforeEach(() => {
    sinon.replace(logger, '$l', sinon.fake())
    sinon.replace(logger, '$e', sinon.fake())
  ...
  it('should output to console.error', () => {
    ...
    logger('test').e('test')
    expect(logger.$e.calledWith('(test) [e] test')).equals(true)
  })

  it('should output to console.log', () => {
    logger.listen(logger.console)
    logger('test').i('test')
    expect(logger.$l.calledWith('(test) [i] test')).equals(true)
  })
})
