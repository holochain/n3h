...
const DhtEvent = createEventSpec({
  ...
  /**
   * Tell implementors to broadcast an entryAspect to all the peers
   */
  dataBroadcast: (dataAddress, dataItem) => {
    type.assert.base64String(dataAddress)
    type.assert.base64String(dataItem)
    return { dataAddress, dataItem }
  },
