...
const { AsyncClass, $sleep } = require('../n3h-common')
...
class P2pBackendMock extends AsyncClass {
  ...
  requestReliable (msgId, list, data) {
    return this._iface.$emitEvent(P2pEvent.handleRequest('abcd', msgId, 'efgh'))
  }
  respondReliable (msgId, fromPeerAddress, data) {
    this._iface.$checkResolveRequest(P2pEvent.handleRequest(
...
describe('P2p Spec Suite', () => {
  ...
  beforeEach(async () => {
    e = []
    p = (await new P2pBackendMock({ e })).interfaceP2p
