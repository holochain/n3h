...
const { encodeId, decodeId } = require('../hc-dpki')

const tweetlog = require('../tweetlog')
const log = tweetlog('@fullsync-gossip@')
...
/**
 * Encode peerId into a gossip buffer
 */
...
/**
 * Decode gossip buffer into encoded peerId
 */
...
function encodeDataMap (bufferList, map) {
  ...
  for (let [a, d] of iterMap(map)) {
    ...
    const b64 = Buffer.from(d, 'base64')
    log.t('############### encodeDataMap', a, Buffer.byteLength(b64))
    datalist.push(b64)
  }
  bufferList.push(keylist)
  bufferList.push(datalist)
