...
const { encodeSign, decodeSign } = require('../hc-dpki')
...
function encodePeerAddress (peerAddress) {
  const signPubId = decodeSign(peerAddress)
  return Buffer.from(signPubId)
}

/**
 */
function decodePeerAddress (signPubId) {
  return encodeSign(signPubId)
}
