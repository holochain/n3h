...
class DhtFullSync2 extends AsyncClass {
      ...
      case 'wantData':
        ...
        for (let addr of res.dataAddressList) {
          if (this._dataMap.has(addr)) {
            resp[addr] = Buffer.from(
              await this.fetchDataLocal(addr), 'base64').toString()
          }
        }
        this._iface.$emitEvent(DhtEvent.gossipTo([task.fromPeerAddress],
      ...
      case 'wantDataResp':
        for (let k in res.data) {
          await this._iface.$emitEvent(DhtEvent.dataHoldRequest(
            k, Buffer.from(res.data[k]).toString('base64')))
        }
        break
      default:
  ...
  async _onDataFetchResponse (task) {
    this._dataFetchTrack.resolve(task.msgId, task.data)
  }
