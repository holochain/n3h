...
class DhtBackendFullsync extends AsyncClass {
  ...
  async _onRemoteGossipBundle (task) {
    ...
    switch (parsed.type) {
      ...
      case 'fetchAddressList': {
        ...
        const dataMap = this._gatherDataMap(parsed.dataAddressList)
   ...
   */
  _gatherDataMap (entryAddressList) {
    const out = new Map()
    for (let entryAddress of entryAddressList) {
      ...
      let entryWithMeta = this.fetchDataLocal(entryAddress)
  ...
  _handleDataMap (dataMap) {
    for (let [address, entryWithMeta] of dataMap) {
      ...
      const evt = DhtEvent.dataHoldRequest(address, entryWithMeta)
      this._spec.$emitEvent(evt)
      // }
    }
  }
