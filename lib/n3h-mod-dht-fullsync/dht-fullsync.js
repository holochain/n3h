...
class DhtBackendFullsync extends AsyncClass {
  ...
  async init (initOptions) {
    ...
    this._name = this._nick(this._thisPeer.peerAddress)
    if (initOptions.name) {
      this._name += '-' + initOptions.name
    }
    ...
    this._peerList = []
    // Map of: loc -> (peerAddress -> peerInfo)
    this._peerStore = new Map()
    // Map of: loc -> (dataAddress -> [contentHash])
    this._dataStore = new Map()
    // Map of: loc -> [(peerAddress -> peerInfo), (dataAddress -> [contentHash])]
  ...
  dropPeerLocal (peerAddress) {
    ...
    const succeeded = lRef.delete(peerAddress)
    if (!succeeded) {
      return
    }
    this._calcState()
  ...
  _fetchDataLocal (dataAddress) {
    return new Promise((resolve, reject) => {
      try {
        ...
        this._iface.$emitEvent(DhtEvent.dataFetch(id, dataAddress))
      } catch (e) {
        reject(e)
      }
    })
  ...
  async _gossip () {
    ...
    this._iface.$emitEvent(DhtEvent.gossipTo(
  ...
  async _onPeerHoldRequest (task) {
    ...
    log.t(this._me() + '._dht.peerHoldRequest:', task)
