...
class DhtBackendFullsync extends AsyncClass {
  ...
  async _gatherDataMap (entryAddressList) {
    ...
    for (let entryAddress of entryAddressList) {
      // For each entryAddress, send a fetchData event to my implementor
      // should receive {entry, metas}
      log.t('_gatherDataMap', entryAddress)
      let entryWithMeta = await this._fetchDataLocal(entryAddress)
      // .then((b64) => {
      //   // log.t('_gatherDataMap', entryAddress, b64)
      //   return b64
      //   // out.set(entryAddress, b64)
      //   // log.t('_gatherDataMap out =', out.size)
      // }, (err) => {
      //   log.e('_gatherDataMap ERROR', err)
      //   return Buffer.from(JSON.stringify(null), 'utf8').toString('base64')
      // })
      log.t('_gatherDataMap entryWithMeta', entryWithMeta)
  ...
  /**
   * Add missing hashes received as a Map
   */
  _handleDataMap (dataMap) {
    for (let [address, entryWithMetaB64] of dataMap) {
      ...
      // Add data item
      // entryWithMeta = JSON.parse(entryWithMeta)
      // for (const hash of hashes) {
      log.t('_handleDataMap', address, entryWithMetaB64)
  ...
  /**
   * Add hash of data for a dataAddress in the _dataStore
   */
