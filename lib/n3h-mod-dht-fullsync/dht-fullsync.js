...
class DhtBackendFullsync extends AsyncClass {
  ...
  getPeerLocal (peerAddress) {
    ...
    console.log('getPeerLocal', peerAddress)

    const loc = getLoc(peerAddress)
    if (!this._peerStore.has(loc)) {
      console.log('this._peerStore.has(loc) failed')
      return null
    }
    ...
    if (!lRef.has(peerAddress)) {
      console.log('lRef.has(peerAddress) failed')
      return null
    }
  ...
  /**
   * Get data from map of: loc -> (peerAddress -> peerInfo)
   */
  ...
  /**
   * Get data from map of: loc -> (entryAddress -> [hash])
   * Creates an empty Map and/or Set on the fly if address is unknown
   */
   ...
   * Most probably reply some other gossip
   */
  async _onRemoteGossipBundle (task) {
    ...
    switch (parsed.type) {
      ...
      case 'fetchAddressList': {
        const peerMap = this._gatherMap(this._peerStore, parsed.peerAddressList)
        const dataMap = this._gatherMap(this._dataStore, parsed.dataAddressList)
  ...
  /**
   * Create map of (address -> content(s)) from a store
   */
  _gatherMap (store, addressList) {
    const out = new Map()
    for (let address of addressList) {
      const loc = getLoc(address)
      if (store.has(loc)) {
        let pRef = store.get(loc)
        if (pRef.has(address)) {
          pRef = pRef.get(address)
          out.set(address, JSON.stringify(pRef))
        }
      }
    }
    return out
  ...
  _handleDataMap (dataMap) {
    for (let [address, hashes] of dataMap) {
      // Skip if we already hold it
      const loc = getLoc(address)
      if (this._dataStore.has(loc)) {
        let pRef = this._dataStore.get(loc)
        if (pRef.has(address)) {
          pRef = this._dataStore.get(loc)
          if (pRef === hashes) {
            continue
          }
        }
      ...
      const evt = DhtEvent.dataHoldRequest(address, hashes)
      this._spec.$emitEvent(evt)
      // }
    }
  }
*/
  ...
  /**
   * Add data to _dataMap
   */
  async _onDataHoldRequest (task) {
    const setRef = this._getDataRef(task.dataAddress)
