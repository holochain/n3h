...
function assertPeerDropRequest (e) {
  if (!DhtEvent.isEvent(e) || e.type !== 'peerDropRequest') {
    throw new Error('expected peerDropRequest DhtEvent, got ' + JSON.stringify(e))
  }
}
...
class Dht extends AsyncClass {
  ...
  async dropPeer (peerAddress) {
    this.$checkDestroyed()
    const out = await this._backend.dropPeerLocal(peerAddress)
    if (out) {
      assertPeerDropRequest(out)
    }
    return out
  }
