...
const { N3hHackMode, N3hMock, N3hRealMode } = require('../lib/index')
...
async function main () {
  ...
  if (process.pid) {
    log.i('This process is your PID ' + process.pid)
  ...
  if (argvOptions.s) {
    log.w('SMOKE TEST - evaluate better-sqlite3')
    const db = new Sqlite('test.sqlite3')
    db.pragma('journal_mode = WAL')
    db.prepare('CREATE TABLE IF NOT EXISTS bob (x INTEGER PRIMARY KEY)').run()
    db.prepare('DELETE FROM bob').run()
    db.prepare('INSERT INTO bob (x) VALUES (?)').run(42)
    db.prepare('INSERT INTO bob (x) VALUES (?)').run(38)
    console.log(db.prepare('SELECT * FROM bob').all())
    db.close()
