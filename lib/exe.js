...
const { debugSnapshotStateMuted, getDebugSnapshot: debugSnapshotGetter } = require('../lib/debug-snapshot')
...
async function main () {
  ...
  -q --quiet   | quiet mode (no debug snapshots)
  -v --version | print version
  -s --smoke   | run smoke test (load up crypto deps && exit)`)
    return process.exit(0)
  }

  const getDebugSnapshot = (argvOptions.q) ? () => debugSnapshotStateMuted : debugSnapshotGetter
